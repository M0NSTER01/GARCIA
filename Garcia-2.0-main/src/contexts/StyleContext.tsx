...
type StyleContextType = {
  ...
  getRecommendationByImage: (imageUrl: string) => Promise<void>;
...
export const StyleProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const mockRecommendationAPI = async (): Promise<StyleRecommendation> => {
    ...
    return {
      bodyType: "Hourglass",
      colorPalette: ["#3a86ff", "#8338ec", "#ff006e", "#fb5607", "#ffbe0b"],
      tops: [
  ...
  const getRecommendationByImage = async (imageUrl: string) => {
    setIsLoading(true);
    console.log('StyleContext: Starting image-based recommendation process');
    ...
    try {
      ...
      try {
        const geminiResult = await analyzeImage(imageUrl, gender);
